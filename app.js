...
function assetForm(a, draftOverride) {
  ...
  const html = `
    ...
    <div id="m_qty" ${a.mode === 'qty' ? '' : 'hidden'}>
      ...
      <p class="hint" id="srcHint">자동으로 설정하면 새로고침 시 시세를 받아옵니다. 국내주식·ETF·미국주식·금은 별도 키 없이 조회됩니다.</p>
...
function symHint(src, cat) {
  if (cat === '원자재' && src === 'twelvedata') return '무료 시세 서버에서 금 1그램당 원화 가격을 조회합니다. 국제 금시세를 환산한 값이므로 KRX 금시장 종가와는 차이가 있을 수 있습니다.';
